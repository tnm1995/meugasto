...
if (!apiKey) {
  console.warn("API Key is missing. Features relying on Gemini API will not work.");
}

// Inicializa o cliente Gemini
// Note: We use an empty string if apiKey is missing to allow app initialization, 
...
export const extractExpenseFromImage = async (base64Image: string): Promise<Omit<Expense, 'id'>> => {
  ...
  try {
    ...
    if (errorMessage.includes('Generative Language API has not been used') || error.status === 403 || errorMessage.includes('PERMISSION_DENIED') || errorMessage.includes('API key not valid')) {
        throw new Error('API_NOT_ENABLED');
    }
...
export const extractTransactionsFromPdfText = async (pdfText: string): Promise<BankTransaction[]> => {
  ...
  try {
    ...
    if (errorMessage.includes('Generative Language API has not been used') || error.status === 403 || errorMessage.includes('PERMISSION_DENIED')) {
        throw new Error('API_NOT_ENABLED');
    }
